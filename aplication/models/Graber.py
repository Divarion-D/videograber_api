...
class Graber_Model():
    ...
    # get_kpun("films", {"order": "NUM_VOTE", "keyword": "Аватар"}))
    def get_kpun(self, param, query):
        url = (
            "https://kinopoiskapiunofficial.tech/api/v2.2/" + param + "?" + urlencode(query)
        )
        # request to url and set header api key
        req = requests.get(url, headers={"X-API-KEY": KPUN_KEY})
        status_code = req.status_code
        resp = req.json_data()

        data = {"status": status_code, "data": resp}
        return data
    ...
    def videocdn_get(self, query, type):
        url = "https://videocdn.tv/api/{}?api_token={}&{}".format(type, VIDEOCDN_API, urlencode(query))
        ...
        if "data" in json_data and json_data["data"] is not None:
            for item in json_data["data"]:
                if self.normalize(item[query["field"]]) == self.normalize(query["query"]):
